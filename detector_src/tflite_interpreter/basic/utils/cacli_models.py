...
def detect_objects(model, IMAGE_PATH, CATEGORY_INDEX, ANCHOR_POINTS, MINIMUM_CONFIDENCE, SAVE_DIR = None):
    ...
    if SAVE_DIR:
      ...
      plt.savefig(output_dir, dpi = 62) #save the image with the detected frame in the output directory
      plt.close(fig)
      print("Image Saved")
      print("="*10)
    ...
    if np.max(scores) > 0.60: # Write to status_ID.txt file to indicate whether a fire is detected
